  ...
  return (
    <div className="bg-zinc-800 rounded-xl p-5 flex flex-col gap-3">
      ...
      <div className="flex gap-2">
        {[
          { value: 'expense', label: 'Gasto', icon: TrendingDown, active: 'bg-red-500 text-white', inactive: 'bg-zinc-700 text-zinc-400 hover:bg-zinc-600' },
          { value: 'income',  label: 'Ingreso', icon: TrendingUp,  active: 'bg-emerald-500 text-white', inactive: 'bg-zinc-700 text-zinc-400 hover:bg-zinc-600' },
        ].map(({ value, label, icon: Icon, active, inactive }) => (
          <button
            key={value}
            onClick={() => set('type', value)}
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-colors ${form.type === value ? active : inactive}`}
          >
            <Icon size={15} />
            {label}
          </button>
